...
open class CollectionDataLoadVC: CollectionVC, DataLoadable {
    ...
    /// 上一次数据总数
    private var collectionItemsCount: Int = 0
    /// 是否显示过加载视图,只显示一次
    ...
    open override func loadView() {
        ...
        if dataRefreshType.isAnyOf([.pullDown, .pullAndLoad]) {
            topRefresher = collectionView.anx.addTopRefresh {
                [weak self] in
                self?.createDataLoad()
            }
        } else if dataRefreshType.isAnyOf([.loadDown, .loadAndPull]) {
            topRefresher = collectionView.anx.addTopAutoRefresh {
                [weak self] in
                self?.pageDataLoad()
            }
        }
        if dataRefreshType.isAnyOf([.pullUp, .loadAndPull]) {
            bottomRefresher = collectionView.anx.addBottomRefresh {
                [weak self] in
                self?.createDataLoad()
            }
        } else if dataRefreshType.isAnyOf([.loadUp, .pullAndLoad]) {
            bottomRefresher = collectionView.anx.addBottomAutoRefresh {
                [weak self] in
                self?.pageDataLoad()
            }
        }
    ...
    deinit {
        ...
        if let collectionView = collectionView {
            collectionView.anx.removeANRHeader()
            collectionView.anx.removeANRFooter()
        }
    }
    ...
    open func pageDataLoad() {
        ...
        loader.cancelWrap = NetworkManager.shared.request(target: target, cache: loader.cache, timeout: loader.timeout, success: {
            ...
            if loader.page == DataLoader.defaultStartPage {
                sSelf.collectionModel.removeAll()
                sSelf.collectionItemsCount = 0
            }
    ...
    private func dataLoadEnd(error: Error? = nil) {
        ...
        let cellItemsCount = collectionModel.sectionItems.reduce(0) { $0 + $1.rows.count }
        ...
        if let error = error as NSError? {
            ...
            if dataLoader.page == DataLoader.defaultStartPage {
                ...
                if dataRefreshType.isAnyOf([.pullDown, .pullAndLoad]) {
                    asyncMainDelay {
                        [weak self] in
                        if let sectionCount = self?.collectionView.numberOfSections, sectionCount > 0 {
                ...
                if dataRefreshType.isAnyOf([.loadDown, .loadAndPull]) {
                    let increasedCount = max(cellItemsCount - collectionItemsCount, 0)
                    asyncMainDelay { [weak self] in
                        self?.collectionView.scrollToItem(at: IndexPath(row: increasedCount, section: 0), at: .top, animated: false)
                    }
                }
            }
            if collectionModel.sectionItems.count > 0 {
                dataLoadState = .success
                if cellItemsCount > self.collectionItemsCount {
                    // 成功,当前有数据,比之前多
        ...
        collectionItemsCount = cellItemsCount
    }
